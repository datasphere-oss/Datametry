{% macro read_configuration_to_sources_yml() %}
    {% if execute %}
            ...
            {% if not table_dict %}
                ...
                {% for table_monitors in table_monitors_list %}
                    {% if 'schema_changes' in table_monitors %}
                        {% do table_dict['tests'].append({'datametry.schema_changes': {'tags': ['datametry']}}) %}
                    {% endif %}
                    {% do table_dict['tests'].append({'datametry.table_anomalies': {'table_anomalies': table_monitors,
                                                                                     'tags': ['datametry']}}) %}
                {% endfor %}
            {% endif %}
            {% if column_name and columns_monitored %}
                {% if column_name ==  '__ALL_COLUMNS__' %}
                    {% for column_monitors in column_monitors_list %}
                        {% do table_dict['tests'].append({'datametry.all_columns_anomalies':
                                                        {'column_anomalies': column_monitors, 'tags': ['datametry']}}) %}
                    {% endfor %}
                {% else %}
                    {% set column_dict = {'name': column_name, 'tests': []} %}
                    {% for column_monitors in column_monitors_list %}
                        {% do column_dict['tests'].append({'datametry.column_anomalies':
                                                        {'column_anomalies': column_monitors, 'tags': ['datametry']}}) %}
                    {% endfor %}
                {% endif %}
            {% endif %}
        ...
        {% do datametry.edr_log(sources_yml) %}
    {% endif %}
{% endmacro %}
