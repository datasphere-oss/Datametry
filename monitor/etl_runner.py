...
class EtlRunner(object):
    DATAMETRY_LOG_PREFIX = 'Datametry: '
    ...
    def _run_command(self, command_args: list, json_logs=False) -> (bool, str):
        ...
        if json_logs:
            etl_command.extend(['--log-format', 'json'])
            capture_output = True
        etl_command.extend(command_args)
        etl_command.extend(['--project-dir', self.project_dir])
        etl_command.extend(['--profiles-dir', self.profiles_dir])
        if self.target:
            etl_command.extend(['--target', self.target])
        logger.info(f"Running {' '.join(etl_command)} (this might take a while)")
        result = subprocess.run(etl_command, check=False, capture_output=capture_output)
        output = None
        if capture_output:
        ...
        if json_logs:
            ...
            for json_message in json_messages:
                ...
                if log_message_data_dict is not None:
                    ...
                    if log_message is not None and log_message.startswith(self.DATAMETRY_LOG_PREFIX):
                        run_operation_results.append(log_message.replace(self.DATAMETRY_LOG_PREFIX, ''))
